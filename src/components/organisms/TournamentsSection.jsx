...
const EventCard = ({ event }) => {
  const isDateLikeLocation = !event.date.match(/^\d/)  && !event.date.match(/^\d{4}$/);
    ...
    <div className="group flex gap-4 items-start py-3 px-4 rounded-xl hover:bg-surface-alt transition-colors duration-200">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-3 mt-1">
          ...
          {/* Badge */}
          {event.badge && (
            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${badgeColors[event.badge] || "bg-royal-50 border-royal/20 text-royal"}`}>
              {event.badge}
            </span>
          )}
        </div>
          ...
          {tournaments.map((yearGroup, yi) => (
            ...
            >
              ...
              <div className="flex-shrink-0 w-[60px] sm:w-[76px]">
                <div className={`
                  flex flex-col items-center justify-center w-full py-2 rounded-xl border font-playfair font-bold text-sm leading-tight text-center
                  ${yi === tournaments.length - 1
                    ? "bg-royal text-white border-royal shadow-brand-sm"
                    : "bg-surface-alt border-border-light text-royal"
                  }
                `}>
                  <span className="text-[10px] font-sans font-semibold tracking-widest uppercase opacity-70">Year</span>
                  {yearGroup.year}
                </div>
              </div>
              ...
              <div className="flex-1 premium-card overflow-hidden">
                ...
                <div className="px-4 py-3 border-b border-border-light flex items-center justify-between bg-surface-alt">
                  <span className="text-xs font-bold tracking-widest uppercase text-ink-muted">
                    {yearGroup.year} · {yearGroup.events.length} event{yearGroup.events.length > 1 ? "s" : ""}
                  </span>
                  {yi === tournaments.length - 1 && (
      ...
      >
        <div className="inline-flex flex-col sm:flex-row items-center gap-4 bg-royal-50 border border-royal/15 rounded-2xl px-8 py-6">
          <div className="text-left">
            ...
            <p className="text-sm text-ink-muted mt-1">Follow us on Instagram or join our WhatsApp community for updates.</p>
          </div>
